...
int main(int argc, char** argv) {
    try {
        if (argc != 3) { std::cerr << "Usage: ./tracker tracker_info.txt <1|2>\n"; return 1; }
        const auto id = p2p::number(argv[2]);
        if (id != 1 && id != 2) throw std::runtime_error("Tracker number must be 1 or 2");
        ...
        if (key.empty() || key.size() > 256) throw std::runtime_error("Invalid P2P_CLUSTER_KEY length");
        p2p::ignore_sigpipe();
        p2p::TrackerServer tracker(static_cast<int>(id), endpoints, journal, key);
        tracker.run();
    } catch (const std::exception& e) { std::cerr << "Tracker error: " << e.what() << '\n'; return 1; }
    return 0;
}
